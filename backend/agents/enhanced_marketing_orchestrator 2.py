...
class EnhancedMarketingOrchestrator(SequentialAgent):
    ...
    def __init__(self, campaign_id: str = None):
        ...
        self.business_analysis_agent = self._create_business_analysis_agent()
        self.content_generation_agent = self._create_content_generation_agent()
        self.visual_content_agent = self._create_visual_content_agent()
        self.social_media_agent = self._create_social_media_agent()
        
        super().__init__(
            name="enhanced_marketing_orchestrator",
            description="Enhanced marketing campaign orchestrator with ADK v1.6.1 features",
            sub_agents=[
                self.business_analysis_agent,
        ...
        logger.info(f"✅ Enhanced Marketing Orchestrator initialized for campaign: {self.campaign_id}")
    
    def _create_business_analysis_agent(self) -> SequentialAgent:
    ...
    def _create_content_generation_agent(self) -> SequentialAgent:
    ...
    def _create_visual_content_agent(self) -> ParallelAgent:
    ...
    def _create_social_media_agent(self) -> SequentialAgent:
    ...
    async def execute_campaign_workflow(
        ...
        try:
            logger.info(f"🚀 Starting enhanced campaign workflow for {self.campaign_id}")
            ...
            self.campaign_state["campaign_context"] = campaign_context
            ...
            logger.info(f"✅ Enhanced campaign workflow completed for {self.campaign_id}")
            ...
            return {
                ...
                "state_summary": self.campaign_state
            }
            
        except Exception as e:
            logger.error(f"❌ Enhanced campaign workflow failed for {self.campaign_id}: {e}", exc_info=True)
    ...
    def get_campaign_state(self) -> Dict[str, Any]:
        """Get current campaign state from in-memory storage."""
        return self.campaign_state
    
    def restore_campaign_state(self, campaign_id: str) -> bool:
        """Restore campaign state from in-memory storage."""
        try:
            self.campaign_id = campaign_id
            # Note: In-memory state doesn't persist between instances
            # This would need to be enhanced with actual persistence
            if self.campaign_state:
                logger.info(f"✅ Campaign state restored for {campaign_id}")
                return True
            else:
